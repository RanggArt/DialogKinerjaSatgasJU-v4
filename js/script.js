...
function cetakDokumen() {
  ...
  // PANDUAN PERBAIKAN: Menggunakan worker html2pdf secara runut & aman
  const worker = html2pdf().set(opsi).from(elemen);
  
  // Jalankan proses simpan PDF
  worker.save();

  // Langsung tutup modal tanpa menunggu promise save yang rentan macet,
  // sekaligus menghindari crash dari method .clean()
  setTimeout(() => {
    tutupModal();
  }, 500); 
}
